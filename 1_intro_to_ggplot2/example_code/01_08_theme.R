...
#   - change plot title: font size 30, face "bold", center alignment, increase below margin 20 points
...
#   - change facets rectangles (titles): color "black", fill color "white", text "bold" with size 16
ggplot(data = df, mapping = aes(x = displ, y = hwy)) +
  geom_point(size = 5, alpha = 1/3) +
  facet_grid(transmission ~ type_of_drive, scales = "free") +
  geom_smooth(method = "lm") +
