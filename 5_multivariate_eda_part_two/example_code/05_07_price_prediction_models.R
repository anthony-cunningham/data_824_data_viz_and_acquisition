...
# Diamond price prediction models

# we are back to building a diamond price prediction model
# some guidelines:
...
# generate all possible combinations of predictors
 ...
 for(nr.predictors in 1:5){ # change number of predictors
  ...
  for(combination in 1:ncol(predictors)){ # loop over every combination of predictors
     ...
     formula <- paste0("price~",paste0(predictors[,combination], collapse = "+")) # model formula
...
set.seed(123)
id.rows <- 1:nrow(diamonds.big) # all rows ids
id.train <- sample(x = id.rows, size = round(0.8 * nrow(diamonds.big)), replace = F) # train rows
...
# # write back to data frame; add a new col name sample
 diamonds.big[id.train,"sample"] <- "train"
 diamonds.big[id.test,"sample"] <- "test"
#check the test and train size
 diamonds.big%>% count(sample)
...
ggsave(filename = "G:/mihaela/TEACHING/05_model_performance_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
