...
# Histogram
# (explore continuous variables like price)
#when we used ggplot with aes, we need one coordinate only
...
#we can control the histogram with the binwidth; change binwidth=1000 to see the 
#difference
diamonds.small %>% 
  ggplot(aes(x = price)) +
...
ggsave(filename = "G:/mihaela/TEACHING/03_histogram_price_diamonds.png", plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
...
# adjust parameters
#adjust=1; more details with lower value; try 1
#size of the line
#density plot color with the fill
diamonds.small %>% 
  ggplot(aes(x = price)) +
  geom_density(adjust = 1/5, linetype = "dashed", size = 1.2, color = "red", fill = "gray")

# final diamond price density plot (for exporting)
#similar as histogram but much smoother
#try different colors for the fill; like red
diamonds.big %>% 
  ggplot(aes(x = price)) +
...
ggsave(filename = "G:/mihaela/TEACHING/03_density_price_diamonds.png", plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
