...
#Data is to big so we can separated in different size to work with 
#easily. We use random number for selection for rows. F - is to not 
#have duplicates
...
# Dotplot
# (geom_dotplot is to explore continuous variables)
...
# different directions of stacks; to represent the plot up or down
diamonds.small %>% 
  ggplot(aes(x = carat)) +
...
#stack ratio- how close are the dots; try 0 and 1
#dotsize-how big the dot is; try 5; 
diamonds.small %>% 
  ggplot(aes(x = carat)) +
...
#we decrease the binwidth because we are working with the big data
#try different color for color="grey20" or "red"
diamonds.big %>% 
  ggplot(aes(x = carat)) +
  geom_dotplot(binwidth = 0.001, stackdir = "up", color = "grey20") +
...
ggsave(filename = "G:/mihaela/TEACHING/03_dotplot_carat_diamonds.png", plot = last_plot(),
