...
# Area plot
# (geom_are() is to explore continuous variables)
...
#  changing statistics to compute area plot for depth

# compute bins and show counts
diamonds.small %>% 
  ggplot(aes(x = depth)) +
  geom_area(stat = "bin", binwidth = 0.02)

# compute density: distribution functions for the density; show density
diamonds.small %>% 
  ggplot(aes(x = depth)) +
...
plot_grid(fig.depth, fig.table, ncol = 1)
