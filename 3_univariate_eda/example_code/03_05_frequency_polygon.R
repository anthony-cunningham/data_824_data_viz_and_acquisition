...
# Density plot
# (geom_density() and geom_freqpoly() are to explore continuous variables)
...
# explore diamond dimensions x / y / z (in mm)

summary(dplyr::select(diamonds.big, x, y, z))
diamonds.big %>% dplyr::select(x,y,z) %>% summary()
...
plot_grid(fig.x, fig.y, fig.z, ncol = 1, labels = c("X", "Y", "Z"))

ggsave(filename = "G:/mihaela/TEACHING/03_freqpoly_subplot_xyz_diamonds.png", plot = fig.subplot,
       units = "cm", width = 29.7, height = 21, dpi = 600) 
############################################################################
...
ggsave(filename = "G:/mihaela/TEACHING/03_freqpoly_xyz_diamonds.png", plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
