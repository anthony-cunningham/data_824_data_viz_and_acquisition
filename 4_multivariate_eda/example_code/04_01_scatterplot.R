...
# parameter for controlling point size
diamonds.small %>% 
  ggplot(aes(x = carat, y = price)) +
  geom_point(size = 3)

# parameter for controlling point shape
diamonds.small %>% 
  ggplot(aes(x = carat, y = price)) +
  #geom_point(shape = 1)
  # geom_point(shape = 2)
...
# parameter for controlling point color
diamonds.small %>% 
  ggplot(aes(x = carat, y = price)) +
  # geom_point(color = "red")
  geom_point(color = "deepskyblue2")

# Shapes with border; try size=5; stroke=5
diamonds.small %>% 
  ggplot(aes(x = carat, y = price)) +
  geom_point(shape = 22, 
             color = "black", 
...
ggsave(filename = "G:/mihaela/04_scatterplot_price_carat_diamonds.png", 
       plot = last_plot(),
