...
# Load data
load("G:/mihaela/TEACHING/diamonds.RData")
...
# (2 continuous variables comparison)
#Two dimensional bin and hex plots are alternative ways to represent the 
#joint density of points in the Cartesian plane.
#The density plot, which we introduced as a visualization for univariate data, 
#can be extended to two-dimensional data.
...
#A hex plot is similar to a 2D bin plot but uses hexagonal regions instead of squares. 
#Hexagonal bins are useful because they can somtimes avoid visual artefacts sometimes apparent with square bins
...
ggsave(filename = "G:/mihaela/TEACHING/04_hexaheatmap2dbin_price_z_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
