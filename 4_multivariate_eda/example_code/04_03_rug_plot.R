...
#Default is off (FALSE). You will also need to use coord_cartesian(clip = "off"). 
#When set to TRUE, also consider changing the sides argument to "tr".
...
# controling line jitter (to avoid overplotting) and transparency
diamonds.small %>% 
  ggplot(aes(x = carat, y = price)) +
  geom_point() +
...
ggsave(filename = "G:/mihaela/TEACHING/04_rugplot_price_carat_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
