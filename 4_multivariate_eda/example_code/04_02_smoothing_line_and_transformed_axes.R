...
# idea: build a linear model where price is dependent variable and carat is independent variable
...
lm.model.price
d<-summary(lm.model.price)


# generate points for drawing a regression line


#build a regression line and define points for which the regression line will be draw
df.lm.line <- data.frame(carat = seq(0,3,0.01)) # df where we store carat and predicted price
#. from all data
#just grep the carat values from df.lm.line
...
# (it found gam - general additive model)
...
ggsave(filename = "G:/mihaela/TEACHING/04_scatterplot_logprice_logcarat_smoothed_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
