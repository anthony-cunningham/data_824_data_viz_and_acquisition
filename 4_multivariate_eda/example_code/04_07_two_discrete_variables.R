...
# here we count cases in each subgroup defined by both discrete variables
...
# position = "fill" (height is normalized from 0 to 1)
diamonds.small %>% 
  ggplot(aes(x = cut, fill = color)) +
...
ggsave(filename = "G:/mihaela/TEACHING/04_barplot_cut_color_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
...
ggsave(filename = "G:/mihaela/TEACHING/04_scatterplot_color_clarity_diamonds.png", 
       plot = last_plot(),
       units = "cm", width = 29.7, height = 21, dpi = 600) 
