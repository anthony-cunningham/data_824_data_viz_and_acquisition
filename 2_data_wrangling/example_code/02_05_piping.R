...
# 2.4 pipe operator to change the command %>%

#   Count the rows where carrier is American Airlines" (AA)
df.AA <-df %>% 
...
df.AA <- df %>% 
  filter(UniqueCarrier == "AA") %>% 
  dplyr::select(UniqueCarrier, Year, Month, DayofMonth, FlightNum, Origin, Destination)
