...
df.long <- df %>% 
  ...
  dplyr::select(DOW = DayOfWeek, Carrier = UniqueCarrier, Distance) %>% 
...
#we want two characters since our Month and DayofMonth has two characters
...
df.date <- df %>% 
  dplyr::select(Year, Month, DayofMonth) %>% 
