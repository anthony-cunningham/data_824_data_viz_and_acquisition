...
library(dplyr)
library(tidyr)
library(tibble) #convert table to tibble
...
dplyr::select(df, Year, Month, DayofMonth)
df.date <- dplyr::select(df, Year, Month, DayofMonth)
rm(df.date)

#   Extract columns that begin with "Taxi"
df.col_taxi<-dplyr::select(df, starts_with("Taxi"))
rm(df.col_taxi)
#   Extract columns that contain "Time"
df.col_time<-dplyr::select(df, ends_with("Time"))
rm(df.col_time)
...
#   Sort rows by the column Distance
